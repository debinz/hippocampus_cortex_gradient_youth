...
dir <- "./Code"
result_dir <- "./Results/gradient_indiv/Mye_bins"
...
for (bin_num in bin_nums) {
  ...
  MeasureL <- paste(result_dir, "/",label,"/myelin_", bin_num, "bins_",label,"_L.csv", sep = "") %>% read.csv(header = FALSE)
  MeasureR <- paste(result_dir, "/",label,"/myelin_", bin_num, "bins_",label,"_R.csv", sep = "") %>% read.csv(header = FALSE)
  ...
  for (bin in 1:bin_num) {
    ...
    #Age_pred <- with(Data, 
    #                expand.grid(Age=seq(min(Age), max(Age), length = length(Measure))))
    Age = mod_gam$model$Age
  ...
  filepath2<-paste(Fig_dir, "/Mye_", bin_num, "bins_age.pdf", sep = "")
