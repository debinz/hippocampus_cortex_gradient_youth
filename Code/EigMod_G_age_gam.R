...
Fig_dir <- file.path(result_dir, "Dev_figure")
...
Measures <- paste(result_dir, "/feat_Eig_G.mat", sep = "") %>% readMat()
Feat_G_L<-Measures["feat.Eig.G"][[1]][[1]][[1]]
Feat_G_R<-Measures["feat.Eig.G"][[1]][[2]][[1]]

p_value<-matrix(NA,nrow=3,ncol=1)
F_value<-matrix(NA,nrow=3,ncol=1)
delta_R_sq<-matrix(NA,nrow=3,ncol=1)
anova_pvalue<-matrix(NA,nrow=3,ncol=1)

for (m in 1:3) {
  
  Feat_G <- c(Feat_G_L[,m],Feat_G_R[,m])
  Feat_G <- data.frame(Feat_G)
  names(Feat_G) <- paste0('EigMod_G',m)
  Data <- cbind(Data, Feat_G)
    
  y1=Data[,paste0('EigMod_G',m)]
  # build GAM model
  ...
  p_value[m,] <- G_summary[["s.table"]]["s(Age)","p-value"]
  F_value[m,] <- G_summary[["s.table"]]["s(Age)","F"]
  ...
  delta_R_sq[m,] <- G_summary$r.sq-G0_summary$r.sq
  ### effect direction
  ...
  if(mean.derivative < 0){ #if the average derivative is less than 0, make the effect size estimate negative
    delta_R_sq[m,] <- delta_R_sq[m,]*-1}
  
  anova_pvalue[m,] <- anova.gam(fit_G0,fit_G,test='Chisq')$`Pr(>Chi)`[2]
  
  #visualization
  myplot <- visreg(fit_G, "Age", gg=T, type="conditional", scale = "response", 
                   overlay=T, partial=T, rug=F, ylab=paste("Corr(EigMod_G", m, ")", sep = ""))
  ...
  ggsave(filename=paste("GeoEigMod_G", m, "_age.pdf", sep = ""), plot=myplot, 
         path=Fig_dir, width = 60, height = 60, units = "mm")
  
  # derv_path<-paste(Fig_dir, "/derv/EigMod_G", m, "_derv_feat_age.csv", sep = "")
  # write.csv(derv, derv_path, row.names = F, quote = F)
}
...
filepath<-paste(Fig_dir, "/EigMod_G_age.csv", sep = "")
