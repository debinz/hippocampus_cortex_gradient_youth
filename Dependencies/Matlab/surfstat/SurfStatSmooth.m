...
Y1=accumarray(edg(:,1),2,[v 1])'+accumarray(edg(:,2),2,[v 1])'; %%zeng: compute 2*degree of each vertex
...
for i=1:n
    ...
    for j=1:k
        if isnum
            Ys=squeeze(Y(i,:,j));
            for iter=1:niter
                Yedg=Ys(edg(:,1))+Ys(edg(:,2)); % Sum of signals of vertices at both ends of each edge (zeng)
                Ys=(accumarray(edg(:,1),Yedg',[v 1]) + ...
                    accumarray(edg(:,2),Yedg',[v 1]))'./Y1;  % Sum of the signal at a paticular vertex with that of all its adj vertices,and divided by (2*degree) (zeng)
            end
            Y(i,:,j)=Ys;
        else
            if length(s)==2
