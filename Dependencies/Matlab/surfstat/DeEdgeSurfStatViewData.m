function [ a, cb ] = DeEdgeSurfStatViewData( data, surf, title, background );
...
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
    double(data(vl)),'EdgeColor',[0,0,0]);
view(-90,0); 
...
if cut<t
    ...
    trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
        double(data(vr)),'EdgeColor',[0,0,0]);
    view(90,0);
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    lighting gouraud ; material dull; shading interp;
else
